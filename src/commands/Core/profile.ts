...
export class UserCommand extends Command {
  public override registerApplicationCommands(registry: ApplicationCommandRegistry) {
    const builder = new SlashCommandBuilder()
      ...
      .addUserOption((option) => option.setName('user').setDescription('This is the user of whos profile you wish to fetch'));
    ...
    const embed = new MessageEmbed()
      ...
      .addField(
        ...
        `User ID: \`${member?.id}\` \n User Tag: \`${member?.user.tag}\` \n Created At: <t:${Math.round(
          member?.user.createdTimestamp ?? 0 / 1000
        )}:R> \n Joined At: <t:${Math.round(member?.joinedTimestamp ?? 0 / 1000)}:R>`
      )
      ...
      .addField(
        'Guild Stats:',
        `Total Messages: \`${(await interaction.channel!.messages.fetch()).filter((m) => m.author.id === member?.id).size}\` \n`
      );
